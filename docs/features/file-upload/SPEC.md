...
1. 관리자 또는 회원이 파일 첨부가 필요한 화면에서 파일을 선택한다.
...
1. 회원이 게시글 저장 시 첨부 파일 ID를 함께 전달한다.
...
| #   | 동작                  | 엔드포인트                                 | 권한   |
| --- | --------------------- | ------------------------------------------ | ------ |
| 4.1 | 업로드 URL 발급(관리) | `POST /api/v1/admin/files/upload-url`      | 관리자 |
| 4.2 | 업로드 확인(관리)     | `POST /api/v1/admin/files/:fileId/confirm` | 관리자 |
| 4.3 | 업로드 URL 발급(회원) | `POST /api/v1/files/upload-url`            | 회원   |
| 4.4 | 업로드 확인(회원)     | `POST /api/v1/files/:fileId/confirm`       | 회원   |

> 고아 파일 정리(크론)·아키텍처(Port/Adapter·OHS)·개발용 Mock 업로드는 api-spec.md 하단 참조.

## 5. 범위

### 포함

- Presigned URL 발급 / 업로드 확인 API (관리자 `/admin/files`, 회원 `/files` — 게시글 첨부 등)
