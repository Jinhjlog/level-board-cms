...
```
A.  SPEC + API계약 → docs/features/<모듈>/{SPEC.md, api-spec.md} ── ✋ 게이트1 (사람)
...
| 단계            | 주체                 | 산출물                                                     | 스킬/에이전트                                     |
| --------------- | -------------------- | ---------------------------------------------------------- | ------------------------------------------------- |
| A SPEC          | 사람(에이전트 보조)  | `SPEC.md` + `api-spec.md`                                  | —                                                 |
| B DB            | 사람(에이전트 보조)  | schema + migration                                         | —                                                 |
| B' 티켓         | 사람                 | 이슈 트래커 에픽/스토리/태스크                             | —                                                 |
| C 컨텍스트      | 에이전트             | (조건부) `dev/context-notes.md`                            | —                                                 |
| D 계약+Mock     | 에이전트             | `presentation/`, `<모듈>.module.ts` (api-spec.md 계약대로) | `presentation-layer`, `swagger-bot`               |
